# Step 1 — Write Salmon quant.sf files 
write_quant <- function(df, path) {
  # Ensure correct Salmon column structure
...
# Step 2 — Simulate RNA-seq quantification   
simulate_quant <- function(path, condition){
  ...
  # KO samples — simulate PTPN22 loss
  if(condition == "KO"){
    TPM[100]      <- runif(1, 0, 0.5)
    NumReads[100] <- rpois(1, 2)
  }
  
  # Build quant.sf table
  quant <- data.frame(
    Name            = genes,
    Length          = Length,
...
# SECTION 3 — Create folders and generate simulated data 
sample_dirs <- c(
  "WT_1","WT_2","WT_3",
...
# SECTION 5 — Load required libaries   
